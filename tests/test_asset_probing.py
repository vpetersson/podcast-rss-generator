...
from rss_generator import _make_http_request, _run_ffprobe_with_retry, get_file_info
...
def _no_sleeping(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("rss_generator.time.sleep", lambda _seconds: None)


def test_http_request_passes_a_timeout() -> None:
    """Without a timeout a hung host stalls the whole feed build."""
    with patch("rss_generator.requests.head") as head:
        _make_http_request("https://example.com/a.mp3")

    assert head.call_args.kwargs["timeout"] == rss_generator.HTTP_TIMEOUT_SECONDS
...
def test_http_request_reraises_after_the_last_attempt() -> None:
    with (
        patch(
            "rss_generator.requests.head", side_effect=requests.ConnectionError("boom")
        ) as head,
...
def test_ffprobe_returns_its_output() -> None:
    ...
    with patch("rss_generator.subprocess.run", return_value=completed) as run:
        assert _run_ffprobe_with_retry("https://example.com/a.mp3") == (
            MOCK_FFPROBE_OUTPUT
        )

    assert run.call_args.kwargs["timeout"] == rss_generator.FFPROBE_TIMEOUT_SECONDS


def test_ffprobe_missing_binary_gives_up_immediately() -> None:
    """ffmpeg not being installed is not a transient failure."""
    with patch("rss_generator.subprocess.run", side_effect=FileNotFoundError) as run:
        assert _run_ffprobe_with_retry("https://example.com/a.mp3") == ""
...
def test_ffprobe_retries_then_returns_empty(error: Exception) -> None:
    with patch("rss_generator.subprocess.run", side_effect=error) as run:
...
def test_get_file_info_survives_an_unusable_probe() -> None:
    ...
    with (
        patch(
            "rss_generator._make_http_request", return_value=make_response(headers={})
        ),
        patch("rss_generator._run_ffprobe_with_retry", return_value=""),
    ):
